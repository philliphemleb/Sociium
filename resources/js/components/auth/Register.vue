<template>
    <div id="register">
        <form class="w-full shadow rounded-lg bg-white px-6 lg:w-1/2 grid grid-cols-2 gap-2 m-auto border-b-2 border-yellow-400" @submit.prevent>
            <h2 class="text-2xl my-4 col-span-2 text-center">{{ $t('auth.register') }}</h2>

            <div class="bg-red-50 border-l-8 border-red-400 mb-2 col-span-2" v-if="errors.length">
                <div class="p-2 grid grid-cols-12 flex items-center">
                    <div class="px-16 col-span-11">
                        <li class="text-md font-bold text-red-500 text-sm" v-for="error in errors">{{ error }}</li>
                    </div>
                    <div class="col-span-1">
                        <div class="ml-2 text-red-400 text-2xl">
            ...
            <div class="flex flex-row col-span-2 md:col-span-1">
                <span class="z-highest rounded-l-lg w-10 h-10 flex justify-center items-center text-2xl text-gray-900 border border-r-0">
            ...
            <button value="button" class="px-4 py-2 rounded bg-green-400 text-white hover:bg-green-900 my-4 col-span-2" @click="submit">{{ $t('auth.register') }}</button>
